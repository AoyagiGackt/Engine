...
{
    if (model == nullptr || model->GetVertexCount() == 0) { return; }
...
float MeshSliceEffect::GetOverlayWeight() const
{
    ...
    if (timer_ < burstStart) { return 1.0f; }
...
void MeshSliceEffect::BuildPieces(const Model* model, std::mt19937& rng, std::vector<SliceVertex>& outVertices)
{
    ...
    if (!srcIdx.empty()) {
        for (uint32_t i : srcIdx) { pushVert(srcVerts[i]); }
    } else {
        for (const auto& v : srcVerts) { pushVert(v); }
    }

    // メッシュ全体の重心と半径（切断位置と運動量のスケール基準）
    Vector3 meshCenter = {};
    for (const auto& v : whole.tris) { meshCenter = meshCenter + Vector3{ v.position.x, v.position.y, v.position.z }; }
    ...
    for (auto& pb : built) {
        ...
        Vector3 c = {};
        for (const auto& v : pb.tris) { c = c + Vector3{ v.position.x, v.position.y, v.position.z }; }
        ...
        if (Length(dir) < 1.0e-4f) { dir = { 0.0f, 1.0f, 0.0f }; }
...
{
    if (!active_ || camera == nullptr) { return; }
...
{
    if (!active_ || pieces_.empty()) { return; }
